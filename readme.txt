...
Contributors: xy440
Tags: seo, google search console, rank tracker, ctr optimization, keyword tracking
...
### 🎯 Key Features & SEO Tools

* **GSC Dashboard Integration** - View Google Search Console analytics directly inside your WordPress admin.
* **Smart SEO Opportunity Detection** - Automatically identifies posts with SERP improvement potential.
* **Money Posts Filter** - Find high-value pages ranking in positions 5-20 with strong impressions.
* **CTR Gap Analysis** - Discover posts with impressions but below-expected organic click rates.
* **Keyword Rank Tracker** - See exact Google rankings and position changes for all your posts.
* **90-Day Performance Data** - Track impressions, clicks, CTR, and average position over time.
* **Keyword-Level Insights (NEW)** - See the top 5 keywords for each post with position and CTR data.
* **Priority Keyword Detection (NEW)** - Smart algorithm identifies the best LSI or primary keyword to target.
...
**What the service is:**
The OAuth proxy (https://auth.theme-x.org/oauth/) facilitates the Google OAuth authentication flow, allowing users to connect their Google Search Console account without creating their own Google Cloud project.
...
**Terms of Service:** https://auth.theme-x.org/terms  
**Privacy Policy:** https://auth.theme-x.org/privacy  
...
= 1.2.1 =
* Fixed: Google Search Console connection failing when using the default OAuth proxy
* Fixed: OAuth authentication errors caused by an expired proxy service domain
* Changed: Updated the default OAuth proxy endpoint
...
= 1.2.1 =
Fixes Google account connection issues for users who could not connect via the default OAuth proxy. Update recommended if you experienced connection errors.
